...
export function ChatInput({
  ...
  return (
    ...
    >
      <InputGroup
        className={cn(
          'focus-within:ring-ring rounded-sm focus-within:ring-1',
          className,
        )}
        {...props}
      >
  ...
  return (
    <>
      ...
      <EditorContent
        editor={editor}
        className={cn(
          'h-full max-h-48 w-full overflow-y-auto px-4 pt-4 pb-2 text-xs',
          className,
        )}
      />
    </>
        ...
        {items.length ? (
          items.map((item, index) => (
            <Button
              ...
              size="sm"
              className={cn(
                'flex justify-start gap-2 px-1 py-2 text-xs',
