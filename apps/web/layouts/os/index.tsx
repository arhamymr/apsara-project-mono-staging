...
import { WindowStateProvider } from '@/layouts/os/WindowStateContext';
import { WindowActionsProvider } from '@/layouts/os/WindowActionsContext';
...
export default function MacOSLayout({ initialAppId }: MacOSLayoutProps = {}) {
  ...
  const { contextValue, stateContextValue, actionsContextValue, windows, activeId } = useDesktopState({
    apps,
    initialAppId,
  });

  // Memoize active window title to prevent unnecessary re-renders
  const activeWindowTitle = useMemo(
    () => windows.find((w) => w.id === activeId)?.title,
    [windows, activeId]
  );

  const content = (
    <WindowProvider value={contextValue}>
      <WindowStateProvider value={stateContextValue}>
        <WindowActionsProvider value={actionsContextValue}>
          <WidgetsProvider>
